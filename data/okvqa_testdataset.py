...
class OkvqaTestDataset(Dataset):
    def __init__(self, config, overfit=False, in_memory=True):
        ...
        img_data = np.load(config["dataset"]["test"]["test_image_features"], allow_pickle=True)
        ...
        sem_data = np.load(config["dataset"]["test"]["test_semantic_graph"], allow_pickle=True)
        ...
        fact_data = np.load(config["dataset"]["test"]["test_facts_graph"], allow_pickle=True)
        ...
        for qid, qa_item in qa_raw.items():
            ...
            self.question_type.append(qa_item['question_type'])
    ...
    def __getitem__(self, index):
        ...
        item['question_type'] = self.question_type[index]
