...
class OkvqaTrainDataset(Dataset):
    def __init__(self, config, overfit=False, in_memory=True):
        ...
        self.questions = []  # question编码
        self.question_lengths = []  # question长度
        self.image_ids = []  # COCO中的image id

        self.fact_num_nodes = []  # fact图中的节点数量
        self.fact_e1ids_list = [] # 起始节点和终止节点
        self.fact_e2ids_list = []
        self.fact_answer_ids = []  # 候选答案表中的答案id
        ...
        img_data = np.load(config["dataset"]["train"]["train_image_features"], allow_pickle=True)
        ...
        sem_data = np.load(config["dataset"]["train"]["train_semantic_graph"], allow_pickle=True)
        ...
        fact_data = np.load(config["dataset"]["train"]["train_facts_graph"], allow_pickle=True)
