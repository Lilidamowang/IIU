...
class IIUCell(nn.Module):
	def __init__(self, 
		device, input_size, hidden_size, num_units, k, rnn_cell, input_key_size = 64, input_value_size = 300, input_query_size = 64,
		num_input_heads = 1, input_dropout = 0.1, comm_key_size = 32, comm_value_size = 100, comm_query_size = 32, num_comm_heads = 4, comm_dropout = 0.1
	):
		super().__init__()
		if comm_value_size != hidden_size:
	...
	def forward(self, question_emb, mems, hs, cs = None):
		"""
		Tips:

		Input:
			question_emb (batch_size, 1, q_size)
			mems len = (batch_size, max_node_num, mem_size)
			hs (batch_size, num_units, hidden_size)
			cs (batch_size, num_units, hidden_size)
		"""
		#hues.info("question_emb : ", question_emb.shape)
		#hues.info("mems : ", mems.shape)
		size = question_emb.size()
		null_input = torch.zeros(size[0], 1, size[2]).float().to(self.device)
		question_emb = torch.cat((question_emb, null_input), dim=1)
		inputs, mask = self.input_attention_mask(question_emb, hs)  # mask为激活矩阵 inputs为att后的输入
		# mask.shape = [11, 5]
		# inputs.shape = [11, 5, in_size(400)]
		h_old = hs * 1.0
		if cs is not None:
			c_old = cs * 1.0

		# self.rnn = GroupLSTMCell(input_value_size, hidden_size, num_units)
		# target = [11, max_node_num, 5, in_size(400)]
		# target = [max_node_num*3, 11, 5, in_size(400)]
		mask_b = mask.unsqueeze(dim=-1)
		mems = torch.split(mems, 1, 1) # [11, 1, 300]
		mems_input = []
		for mem in mems:
			mems_input.append(torch.bmm(mask_b, mem))
		for m in mems_input:
			hs, cs = self.rnn(m, (hs, cs))
		mask = mask.unsqueeze(2)
		h_new = blocked_grad.apply(hs, mask)
...
def IIUTest():
	cell = IIUCell('cpu', input_size=300, hidden_size=100, num_units=5, k=3,
                                            rnn_cell="LSTM")
	hs = torch.randn(11, 5, 100)
	cs = torch.randn(11, 5, 100)
	question_emb = torch.randn(11, 1, 300)
	mems = torch.randn(11, 50, 300)
	hs, cs = cell(question_emb, mems, hs, cs)
	print('finished')